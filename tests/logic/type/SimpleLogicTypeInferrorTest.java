...
public class SimpleLogicTypeInferrorTest {
	...
	public SimpleLogicTypeInferrorTest() {
		...
		universe.put("2");
		inferror = new SimpleLogicTypeInferror<>(asList(
					(nodes, types) -> {
						boolean hasNest = nodes.get(1).getToken().isOfType(OPEN_BRACKET);
						if ("where".equals(nodes.get(hasNest ? 3 : 2).getToken().getValue())) {
							Type type = types.getPassedValues().get(nodes.get(hasNest ? 6 : 5));
							if (type != null) {
								return type;
							}
						}
