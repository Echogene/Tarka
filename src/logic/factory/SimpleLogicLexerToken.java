...
public class SimpleLogicLexerToken extends Token {
	...
	public enum SimpleLogicLexerTokenType implements TokenType {
		...
		public static boolean isTokenOpenCurlyBracket(Token token) {
			return token.isOfType(OPEN_BRACKET)
					&& token.getValue().equals(OPEN_CURLY_BRACKET);
		}

		public static boolean isTokenCloseCurlyBracket(Token token) {
			return token.isOfType(CLOSE_BRACKET)
					&& token.getValue().equals(CLOSE_CURLY_BRACKET);
		}
	}
}
